...
class Trainer:
    def __init__(self, ARCH, DATA, datadir, logdir, path=None, point_refine=False, local_rank=0, debug: bool = False):
        # parameters
        self.ARCH = ARCH
        self.DATA = DATA
    ...
    def set_gpu_cuda(self):
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 1 and not self.debug:
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.ARCH["train"]["max_epochs"]):
            ...
            rand_img = []
            if epoch % self.ARCH["train"]["report_epoch"] == 0 and self.local_rank == 0 and epoch >= self.ARCH["train"]["start_val_epoch"]:
    ...
    def train_epoch(self, train_loader, model, all_criterion, optimizer,
        ...
        for i, (in_vol, proj_mask, all_proj_labels, _, path_seq, path_name,
                _, _, _, _, _, _, _, _, _, polar_data, p2r_matrix) in enumerate(train_loader):
            ...
            # compute output
            output, _, movable_output, _ = model(in_vol, cat_bev_res_fea, unq, unq_inv, batch_size, cur_dev, p2r_matrix,
                                                 train_mode=True)
    ...
    def validate(self, val_loader, model, all_criterion, all_evaluator, class_func, color_fn, save_scans=False):
        ...
        with torch.no_grad():
            ...
            non_blocking = True
            for i, (in_vol, proj_mask, all_proj_labels, _, path_seq, path_name,
                    _, _, _, _, _, _, _, _, _, polar_data, p2r_matrix) in enumerate(tqdm(val_loader, desc="Validation:", ncols=80)):
                proj_labels, movable_proj_labels = all_proj_labels
                ...
                if self.gpu:
                    proj_labels = proj_labels.cuda(non_blocking=non_blocking).long()
                    movable_proj_labels = movable_proj_labels.cuda(non_blocking=non_blocking).long()

                    ### bev
                    bev_residual_data = bev_residual_data.cuda(non_blocking=non_blocking)
                    xy_ind = xy_ind.cuda(non_blocking=non_blocking)
                    p2r_matrix = p2r_matrix[:, :, :, :2].cuda(non_blocking=non_blocking)

                cat_bev_res_fea, unq, unq_inv, batch_size, cur_dev = self.polar_preprocess(bev_residual_data, xy_ind, num_pt)

                # compute output
                output, _, movable_output, _ = model(in_vol, cat_bev_res_fea, unq, unq_inv, batch_size, cur_dev, p2r_matrix, train_mode=False)
