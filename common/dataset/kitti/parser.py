...
class SemanticKitti(Dataset):
    ...
    def __init__(self, root, # directory where data is
                 ...
                 bev_residual_path: Optional[str] = r"/data/2022/mos-dataset/Apollo/apollo_bev_res/"
    ):
        # save deats
        ...
        for index in range(start_index, start_index + 1):
            ...
            if self.gt:
                scan = SemLaserScan(self.color_map,
                                    ...
                                    bev_residual_data=bev_residual_data
                                    )
            else:
            ...
            if not self.transform:
                assert scan.points.shape[0] == bev_residual_data.shape[0]

            assert scan.points.shape[0] == scan.bev_residual_data.shape[0]
            polar_data, polar_point2pixel = self.polar_dataset(scan.points, label_file, residual_data=scan.bev_residual_data)
